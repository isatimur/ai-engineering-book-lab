...
Robotics runs the same playbook against a harder clock, and it is worth reading that way concretely: take each voice primitive from the sections above and find its physical analog.
...
The one variable that changes the engineering between them is irreversibility. A voice agent that picks the wrong filler can recover in the next turn; a robot that commits the wrong motion cannot un-commit it. So robotics pushes the same masking-and-tuning machinery toward the conservative end: actions stay bounded and reversible where possible, and the agency dial is set lower the more a mistake costs to undo — the same risk-versus-irreversibility logic that governs how much autonomy any high-stakes agent is given.
...
That is also the chapter's connection back to the rest of the book. Chapter 3 argued that the harness around the model determines what a coding agent can be trusted to do. Chapter 4 argued that evals are a control system, not just a test suite. Chapter 5 argued that context quality is mostly a problem of assembly, not stuffing. Chapter 6 argued that durable state and workflow semantics are trust features, not backend details. Each of those claims sharpens under realtime pressure. Each becomes audible. Voice is the substrate where the book's frame stops being inferable and starts being heard.
...
The last word in the voice cluster belongs to Zeghidour, who has been honest enough throughout his talk to keep saying that there is more to do. "The last mile is going to be the most difficult to solve," he says, near the end. The chapter would not put it differently. The first mile of voice was a model problem — speech recognition, synthesis, and the language model itself, all now fast. The last mile is an infrastructure problem, and it has a concrete address: the orchestration that holds the end-to-end turn budget, the masking layer that keeps the conversation alive through tool-call variance, and the duplex ceiling that decides whether overlap is possible at all. The first one is mostly solved. The second one is the work, and it is the same work the rest of this book has been describing.
