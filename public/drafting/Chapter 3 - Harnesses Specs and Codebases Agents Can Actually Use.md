...
## Draft note
Drafting mirror added to complete the tracked public/drafting layer (ch1/2/4/5/6/7/8/9/10
already had mirrors; ch3 was the only gap). Body below is the verbatim published prose
from `website/src/content/chapter-03.md` after the 2026-06-01 scaffold-leak fix. Source
cluster per Chapter Packet v1. Reuses existing ledger claims #3, #4, #5, #6, #7, #14, #17
(harness/spec/codebase/factory cluster); no new claims registered in this mirror pass.

The argument arc:
1. When coding agents disappoint, the environment is usually the real suspect, not the model.
2. The software-factory vignette — quality gets erratic until the workplace is made legible.
3. The repo is the real interface; the prompt is the visible tip.
4. Good code contains hundreds of unstated decisions — specs externalize them.
5. Specs are executable intent; agent-ready codebases are designed, not discovered.
6. The harness is a workflow, not just a wrapper; subagents are harness components.
7. The new advantage is environment design — hand off to Chapter 4 (is the work good?).
