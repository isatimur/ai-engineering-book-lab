...
|---|---|---|
| P1 | **Full-coverage MASH run — fix implemented, re-run gated.** Run 4 (`0cb7`) confirmed the cause is sustained, not transient: usefulness/voice/redundancy/evidence_density hit **100%** but humanness (**70%**) and claim_defensibility (**71%**) still gapped under the `03806a3` retry layer, because the two biggest-prompt judges (humanness bundles surrounding paragraphs; claim_defensibility bundles the ledger) blow the per-key TPM ceiling faster than backoff recovers. **Fixed** in book-mash `3926f25`: a second, tighter semaphore (`_HEAVY_CONCURRENCY=1`) serializes just those two judges — acquired before the global slot (no deadlock) — so they drip under the ceiling while the small-prompt judges keep full `_CONCURRENCY=3` throughput (regression test asserts heavy peak ≤ cap; suite 67 green). Remaining: one re-run to confirm humanness/claim_defensibility reach ~100% (gated on the API key; ~$0.60 warm) | book-mash `runners/measurement.py` · `99_Meta/scripts/build_judge_scores.py` |
