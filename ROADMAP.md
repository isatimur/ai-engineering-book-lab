...
|---|---|---|
| P1 | **Full-coverage MASH run — retry fix helped but did NOT fully close the gap.** Run 4 (`0cb7`, 2026-06-10, $0.61 warm cache) with the `03806a3` retry fix + `932d073` skip_sections in place: usefulness/voice/redundancy/evidence_density now **100%**, but humanness (**70%**) and claim_defensibility (**71%**) *still* gap. So the cause is not purely transient 429s — the two biggest-prompt judges (humanness bundles surrounding paragraphs; claim_defensibility bundles the ledger) are hitting a harder per-key TPM ceiling. Next lever: per-dimension request batching / a lower `_CONCURRENCY` for just those two judges, or a higher-tier key | book-mash `judges/_retry.py` · `99_Meta/scripts/build_judge_scores.py` |
