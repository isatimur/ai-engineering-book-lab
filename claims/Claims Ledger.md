...
- **Supporting sources:**
  ...
  - [[625-0n3MKk7r60w-lessons-from-scaling-github-s-remote-mcp-server-sam-morrow-github|#625 — Sam Morrow, GitHub]]
    ...
    - **Currency checked (2026-09-14):** no newer figure exists — #992 and #761 return zero hits for `[0-9]+% (reduction|fewer|less)|token (reduction|savings)|initial load`, and a sweep of every talk added after this source for `github('s)? (remote )?mcp server` found only two passing mentions, neither restating a number. **But the figure must be dated wherever it is used.** The speaker disclaims it on both sides: *"you don't need to worry too much about the specifics. This is dated now."* (05:10) immediately before, and *"it's it's it's a moving target. We're constantly changing things that improve it."* (06:16) after. He also supersedes it inside his own talk by grouping CRUD tools, and the tape says *"like about 49%"* — the hedge sits at 00:05:32.360, just outside the anchor, so the number reads more precise than he made it. Cite as "GitHub's remote MCP server, spring 2026, one intermediate step the speaker called dated", never as a stable benchmark.
...
- **Supporting sources:**
  - [[653-ow1we5PzK-o-the-multi-agent-architecture-that-actually-ships-luke-alvoeiro-factory|#653 — Luke Alvoeiro, Factory]]
    - **Anchor:** `ow1we5PzK-o` 00:09:26.560 → 00:09:36.440 · confidence: high
    - **Quote:** "If you have 10 agents running at one point in time then you have 10 times the throughput. But we tried that and it doesn't really work for tasks in the like software dev domain because agents conflict."
    - **Anchor:** `ow1we5PzK-o` 00:09:37.400 → 00:09:48.880 · confidence: high
    - **Quote:** "They step on each other's changes. They duplicate work. They make inconsistent architectural decisions. And so the coordination overhead ends up eating up the speed gains all the while you're burning tokens."
    - **Mis-filed source replaced (2026-09-14):** the anchor here was the same 19-second opening promise this ledger already removed from `claims#11` on 2026-09-04 — *"you'll be able to assemble agent teams that can complete tasks orders of magnitude harder than what you can complete with a single agent today."* It was struck from one entry and left standing in another, which is the copied-defect pattern that put one audience question into two entries. Worse here: the anchor begins at "you'll", and the words immediately before it are *"my name is Luke and my goal is that 20 minutes from now"* (00:00:17.920). Truncation turned a talk-agenda promise into what reads as an industry forecast — which is also why a currency review flagged it as a time-bound prediction. It is not a prediction at all. The replacement is the passage where Alvoeiro measures throughput against coherence, which is this claim's actual proposition.
    - **Currency checked (2026-09-14):** still current. Three independent sweeps — talks after index 660, after index 800, and finally all 1,074 transcripts — for multi-agent reversal language (`don't build multi-agent`, `multi-agent systems are fragile`, `avoid multi-agent`, `single agent is better`, `multi-agent is a trap`) returned **zero hits at every scope**. No talk in the corpus, at any date, reports trying agent teams and going back to a single well-contexted agent. Note this is consistent with the new anchor rather than in tension with it: Alvoeiro reports naive *parallelism* failing and responds by coordinating agents (running features serially under a coordinator), not by abandoning multi-agent. Confirming later evidence: #866 (Lee Robinson, Cursor, 2026-07) — *"increasingly we find that you have a human working with a team of agents, and then the agents can start working with the other agents"* (17:02).
...
- **Supporting sources:**
  ...
  - [[625-0n3MKk7r60w-lessons-from-scaling-github-s-remote-mcp-server-sam-morrow-github|#625 — Sam Morrow, GitHub]]
    - **Anchor:** `0n3MKk7r60w` 00:05:33.120 → 00:05:36.040 · confidence: high
    - **Quote:** "49% reduction of the initial load."
    - **Currency checked (2026-09-14):** anchor widened — it previously ended at 00:05:35.600, which is where "initial" *starts*, cutting off "load" mid-phrase. Now identical to `claims#15`, which cites the same measurement for a different claim. The same dating caveat applies: see `claims#15`.
...
- **Supporting sources:**
  ...
  - [[172-4Xe_iMYxBQc-information-retrieval-from-the-ground-up-philipp-krenn-elastic|#172 — Philipp Krenn, Elastic]] — retrieval as a system of techniques with known trade-offs, not a black box; misassembly as the cost of treating it as one.
    ...
    - **Currency checked (2026-09-14):** still current. The only time-bound element is "becoming more and more popular", which would need the popularity to *reverse*. It has not — seven talks added after this source still use hybrid search and reranking, the newest three weeks before this review. The trend has completed rather than turned: #1019 (2026-08-20) calls it *"a very classic hybrid search pipeline"* (23:43) where Krenn called it newly popular, and defends it empirically — *"were buried in the middle and it started giving us like 0% recall whereas uh you know something like BM25 it still got 100% every time. So semantic search uh on its own is not enough."* (59:04). #1006 pursues a different bet (training context into weights) but never says reranking is declining; a competing approach is not a declining trend. Checked independently of the sibling quote in this entry that is already marked superseded.
...
- **Supporting sources:**
  ...
  - [[031-AHtGAgQ0Q_Q-why-and-how-you-need-to-sandbox-ai-generated-code-harshil-agrawal-cloudflare|#31 — Harshil Agrawal, Cloudflare]] — sandbox-by-default for any agent executing code; the bounds are the product, not the fallback.
    ...
    - **Currency checked (2026-09-14):** still current. Swept all 1074 transcripts for `each tab|per tab|every tab|site isolation|browser sandbox` — nothing says browsers moved off per-tab isolation. Checked 38 newer notes on sandboxing/isolation and a contradiction pattern (`sandboxing is obsolete|dead|theater|not enough`): 13 hits, all pro-sandbox in context. Two apparent counter-signals resolve as confirming — #979 restates the architecture ("there is a sandbox. And so, inside this sandbox is where you run your untrusted code"), and #899 calls existing *agent* sandboxes "worthless" while building a stronger micro-VM boundary, which strengthens the claim rather than retiring it. #905 (2026-08): "Sandboxes are so hot. So, agents need sandboxes. We kind of like settled upon that at this point in time."
...
- **Supporting sources:**
  - [[065-MGzymaYBiss-dispatch-from-the-future-building-an-ai-native-company-dan-shipper-every-ai-i|#65 — Dan Shipper, Every]] — "there's a 10x difference between an org where 90% of the engineers are using AI versus an org where 100% of the engineers are using AI" — the discontinuity-at-full-adoption framing.
    ...
    - **Currency checked (2026-09-14):** still current, and structurally it cannot have a later edition — a one-off rhetorical multiplier from a single practitioner does not recur, unlike the annual survey marked below. #65 is the only talk in the corpus where Shipper speaks (#1044 is Kieran Klaassen; Every-the-company is not Shipper-the-speaker for edition purposes). Corpus sweep for `10x difference|full adoption|last 10%|100% adoption|everyone using AI` found nothing on point. **Worth knowing:** #785 (Angie Jones, Block) reports an org sitting at exactly Shipper's 90% — *"After a couple of months, about 90% of our engineers were now using tools like Goose and Claude Code regularly to generate code."* (00:44) — and getting no gain: *"he was concerned, because we weren't shipping any faster."* (01:02). That supports the structural argument (partial adoption leaves old workflows intact) but attributes the gap to *how* the tools were used rather than to the missing last 10%. The caveat already says the multiplier is not load-bearing; this reinforces it.
...
- **Supporting sources:**
  - [[761-so9l_MwS2yg-how-to-keep-shipping-when-you-walk-away-from-your-desk-zack-proser-workos|#761 — Zack Proser, WorkOS]] — agents can be given verification criteria and the tools to meet it and then looped infinitely; the human's attention is what degrades under load and stays the hard constraint.
    ...
    - **Currency checked (2026-09-14):** still current, and strengthened — the quote is a prediction, so confirmation is the relevant test. Searched all 280 newer notes for `bottleneck`, `human in the loop|oversight|verif`, and `code review|reviewing`: no source automates human verification away. The two most supersession-shaped titles both stop short. #1041 "The Last Human Code Review" gates its endpoint on a future date (*"We want to reach in 2027…"*), and a 2027 vendor aspiration cannot supersede a 2026 claim. #1021 "How to Kill the Code Review" (Ankit Jain, Aviator, 2026-08-20) is the strongest confirmation found: *"We have just increased the bottleneck to from the coding now kind of like coding is solved to now reviewing, where everything just gets stuck there"* (01:53). His replacement keeps the human — *"So, now reviewers are reviewing the intent, not the diff"* (13:08) — which relocates the constraint rather than dissolving it.
...
- **Supporting sources:**
  ...
  - [[692-BcWFc3H7Khg-let-s-go-bananas-with-genmedia-guillaume-vernade-google-deepmind|#692 — Guillaume Vernade, Google DeepMind]] — the same cost economics as a platform service tier: trade latency for a discount.
    ...
    - **Quote:** "flex and that's basically I don't care if that takes a long time but I want to pay to to pay less. So you're going to have a 50% discount but your request can be uh can be delayed"
    - **Currency checked (2026-09-14):** still current. The quote is a vendor pricing fact (Google's flex tier, 50% discount for latency tolerance). Searched every newer transcript for `deprecat|sunset|discontinu|no longer available|service tier|priority tier|50%` — no withdrawal or repricing anywhere. Re-confirmed at the same magnitude six weeks later by a different vendor's equivalent, #990 (2026-08-14) on Anthropic batch: *"can put them in a batch and for 50% fewer token cost you will get the result they promise in at at least 24 hours."* (18:50) — same economics, same number, so it confirms the pattern rather than superseding the Google tier. Tiered routing is more established, not less: #985 and #1054 are whole panels on shipped routing architectures, and an anti-routing scan of both returned zero hits. **Refinement, not staleness:** #985 (15:31) warns that cheapest-per-token is not cheapest end-to-end on agentic work — *"like Opus will do about three times better at 1/10 the cost of Haiku, even though Haiku's significantly cheaper per token."* That sharpens "the cheapest model that can do it" toward total task cost, and argues for smarter routing, not against it.
...
- **Supporting sources:**
  - [[792-dRmWYHuIJxM-we-cut-94-of-ai-coding-tokens-with-a-local-code-index-rajkumar-sakthivel-tesco|#792 — Rajkumar Sakthivel, Tesco]] — states the cost decomposition directly and argues that fixing the input outweighs model choice; reports a measured (best-case) 94% input-token cut from local code indexing on a benchmark repo.
    ...
    - **Currency checked (2026-09-14):** the split itself stands; see the **Prescription superseded** note on the next anchor for what changed around it.
    ...
    - **Prescription superseded (2026-09-14):** the *quotes* stand — nothing in the corpus revises the input/output cost split, and #1070 (`YXowceUKYJI`, 2026-08-28) independently reports the token ratio is "uh is massive oftentimes over a 100 ratio and even higher" (04:07). But two of the levers this entry recommends around them — "compress" and "summarize tool results" — now have a measured counter-result. #1019 (`WP3hjUXd918`, added 2026-08-20) ran a 36-turn, 1.78M-token comparison and found full history beat compaction on all three axes: *"not touching uh is actually cheaper, it's faster and we have better recall overall. So keeping everything wins on on all of these three fronts"* (45:45). The mechanism is prompt caching — *"the full history u you know setup that we had was sending the most tokens but uh we were still getting the best results out of it because 97% of the tokens that we had were cached"* (52:28) — and compaction breaks the cache, so *"the cheapest run is actually the one which is sending the most tokens"* (01:01:08). #1070 sizes the gap at *"10x cost difference between cash and non-cash tokens"* (06:12). Their own takeaway: *"do not compact by default"* (01:01:41). **Scope:** one team's AI-tutor workload, and they say so; it is a counter-result to compacting by default, not a law. Caching compresses the input-dominance margin but does not flip it — at a 100:1 ratio with a 90% hit rate input is still the majority of the bill. Cache first; compact only against a named constraint.
...
- **Candidate chapters:** 5, 6
- **Reusable phrasing:** In a retrieval agent, most of the bill is what you feed the model, not what it writes back. Fix the input first — retrieve precisely, index instead of dumping files, and cache aggressively — and the choice of model matters less than you think. (Phrasing amended 2026-09-14: this read "cache and compress" until #1019 measured compaction breaking the cache and costing more. Do not print the old version.)
...
- **Supporting sources:**
  - [[315-N4vCBM5YbN0-building-code-first-ai-agents-with-azure-ai-agent-service-cedric-vidal-microsoft|#315 — Cedric Vidal, Microsoft]] — Framework proliferation stated directly: "so many ways to build an agent today so many frameworks" (langchain, langgraph, semantic kernel).
    ...
    - **Currency checked (2026-09-14):** still current — this was the likeliest of the set to decay and did not. Grepped every note added after the source for `consolidat|standardiz|de facto|has won|settled on|converg`: nine hits, none about agent-framework consolidation. The decisive evidence is a framework vendor saying the shakeout has not happened — Sam Bhagwat (Mastra, #908, 2026-07-26): *"still exploring you know the industry is still exploring options"* (08:30), dating it forward at *"and and I think that in the notsodistant future there will be this very real shakeout"* (13:41). #991 (2026-08-14) benchmarks five orchestrator frameworks across four models, which is both halves of the claim in one talk. Note the model-churn half is reinforced by #830 — but that is the *same speaker* as the anchor one month later, so it is reinforcement, not independent corroboration.
