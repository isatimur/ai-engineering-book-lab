...
Two hundred milliseconds is approximately the length of a comfortable conversational gap in English. Anything longer registers, consciously or not, as a hesitation. Two beats and the system feels slow; three and it feels confused.
...
The instinct of many teams is to patch this in product. Add a *wait* button. Add a tone-detection heuristic. Add a longer silence threshold. Each patch helps, marginally, and each patch is paid for in worse responsiveness somewhere else. The half-duplex constraint propagates.
...
Humeau is admirably specific about where his own released model departs from the pattern: it uses diffusion or flow-matching for the per-frame stage rather than full autoregression. The convergence claim is about the backbone, not full-stack. That distinction is worth keeping; it is one of the reasons the claim sits at moderate rather than strong support in the ledger. But the direction is unambiguous, and it shows up beyond Mistral. The same architectural family — autoregressive decoder backbones streaming token-by-token — is now visible across leading voice work.
