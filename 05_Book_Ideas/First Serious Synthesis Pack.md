...
Corpus size analyzed: **1013 videos**
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 979, 'unavailable': 32}**
...
Across the full 1013-video corpus, four things stand out most clearly:
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 382, `Coding Agents`: 468).
- **Reliability and evals** are now too large to be a chapter footnote (`Evals & Reliability`: 418).
- **Org design and leadership** remains a major recurring signal (`Org Design & Leadership`: 247).
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 118, `RAG & Retrieval`: 168).
- **Security/guardrails** and **voice/realtime** are narrower, but strategically important edge-pressure topics (`Security & Guardrails`: 26, `Voice & Realtime`: 44).
