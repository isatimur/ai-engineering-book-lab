...
Corpus size analyzed: **1153 videos**
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 1071, 'unavailable': 79, 'whisper_base_en': 1}**
...
Across the full 1153-video corpus, four things stand out most clearly:
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 495, `Coding Agents`: 552).
- **Reliability and evals** are now too large to be a chapter footnote (`Evals & Reliability`: 486).
- **Org design and leadership** remains a major recurring signal (`Org Design & Leadership`: 255).
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 145, `RAG & Retrieval`: 174).
- **Security/guardrails** and **voice/realtime** are narrower, but strategically important edge-pressure topics (`Security & Guardrails`: 27, `Voice & Realtime`: 53).
