...
Corpus size analyzed: **1125 videos**
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 1071, 'unavailable': 51, 'whisper_base_en': 1}**
...
Across the full 1125-video corpus, four things stand out most clearly:
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 479, `Coding Agents`: 552).
- **Reliability and evals** are now too large to be a chapter footnote (`Evals & Reliability`: 486).
- **Org design and leadership** remains a major recurring signal (`Org Design & Leadership`: 253).
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 143, `RAG & Retrieval`: 168).
