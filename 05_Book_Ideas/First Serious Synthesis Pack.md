...
Corpus size analyzed: **1135 videos**
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 1071, 'unavailable': 61, 'whisper_base_en': 1}**
...
Across the full 1135-video corpus, four things stand out most clearly:
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 486, `Coding Agents`: 552).
...
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 144, `RAG & Retrieval`: 170).
- **Security/guardrails** and **voice/realtime** are narrower, but strategically important edge-pressure topics (`Security & Guardrails`: 27, `Voice & Realtime`: 47).
