...
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 938, 'unavailable': 31}**
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 342, `Coding Agents`: 425).
- **Reliability and evals** are now too large to be a chapter footnote (`Evals & Reliability`: 380).
- **Org design and leadership** remains a major recurring signal (`Org Design & Leadership`: 243).
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 114, `RAG & Retrieval`: 167).
