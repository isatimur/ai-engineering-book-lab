...
Corpus size analyzed: **983 videos**
Transcript status mix: **{'auto_en': 2, 'auto_en_orig': 967, 'unavailable': 14}**
...
Across the full 983-video corpus, four things stand out most clearly:
...
- **Agent architecture + coding systems** dominate the corpus (`Agent Architecture`: 363, `Coding Agents`: 452).
- **Reliability and evals** are now too large to be a chapter footnote (`Evals & Reliability`: 406).
- **Org design and leadership** remains a major recurring signal (`Org Design & Leadership`: 241).
- **Runtime/tooling/context infrastructure** is substantial and growing (`MCP & Tooling`: 117, `RAG & Retrieval`: 166).
- **Security/guardrails** and **voice/realtime** are narrower, but strategically important edge-pressure topics (`Security & Guardrails`: 26, `Voice & Realtime`: 43).
