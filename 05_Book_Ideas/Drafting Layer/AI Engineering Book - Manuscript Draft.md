...
That is a different standard. A helpful answer can be wrong and still useful. Delegated work is expensive in a different way. It consumes time, touches systems, shapes decisions, and often hides its mistakes inside output that looks plausible enough to pass a casual glance. That last property is the real trap: the primary failure surface is not the work that looks broken but the work that looks finished. Rafael Levi at Bright Data names the mechanism in its purest form, describing web-browsing agents that hit a block and do not say so: "There's no error, no warning, just the wrong answer." The agent, he says, "gets blocked, it needs to please you and it makes things up" — the same shape of failure recurs anywhere an agent is allowed to report on its own success: a test claimed to pass, a source claimed to be checked, a step claimed to be done. The moment AI crosses from consultation into execution, eloquence stops being the main thing that matters, and the casual glance stops being an acceptable form of review. What matters is whether the system can produce bounded, inspectable, dependable work — verified by something other than the system's own account of itself.
...
The labels themselves are less important than the operating difference, and the cleanest way to tell them apart is to ask where the human sits relative to the loop: in the critical path for every step, alongside the work in real time, or out of the loop until review. An assistant helps you think; nothing it produces takes effect until you have read it. A copilot lands its contribution inside a task already in motion, the way it completes the line you were already typing. A delegate steps you out of the moment-to-moment loop and back in at review, changing the state of the world even if only by producing work that others will rely on. A fast diagnostic: ask what happens if no human looks at the output for the next 24 hours. An assistant's suggestion goes stale — harmless, ignorable. A delegate's output has usually already changed something by then: a pull request opened, a record updated, a clock already running toward a deadline someone else now inherits. If the 24-hour-unread version of the system looks different from the assistant version, you are already past the assistant stage, whatever the product page calls it.
...
The AI field keeps relearning a painful distinction: demo capability and operational dependability are not the same thing. A model can look extraordinary in a controlled interaction and still fail as a working system. It can write code that seems right but violates a local convention nobody wrote down. It can retrieve relevant documents but miss the one paragraph that actually governs the decision. It can produce a beautiful answer while silently losing track of what happened two steps ago. It can look productive and still be impossible to trust. The test that separates the two is repetition, not a single impressive run: does the same request, phrased three different ways, produce the same dependable behavior when no one is watching closely enough to catch the drift? A demo only has to clear that bar once. A production system has to clear it every time, unobserved. That gap is what AI engineering exists to close.
...
One of the easiest mistakes to make in an AI-heavy moment is to confuse abundance with mastery. A team can now generate ten versions of a feature instead of one. A founder can spin up a prototype in a weekend. A product manager can produce a plausible spec without a formal writing cycle. A junior engineer can land patches that once took far longer. The system seems to remove friction everywhere.

Some of that is real; routine output really has become cheaper. But cheap output does not mean cheap judgment — except where the output is genuinely disposable. For a one-off script or a prototype nobody inherits, skip the judgment overhead. Where the work has to endure, the bill comes back.
...
But fundamentals here are not syntax; the machine fills that in. They are how you tell good output from merely convincing output — the leverage points a machine cannot resolve for you:
...
Artman's is sharper: when a small AI-equipped team can match your feature set, he sees tasteful, high-quality software as a way to keep some competitive advantage.
Speed changes what human excellence consists of.
...
Armin Ronacher and Cristina Poncela Cubeiro make the sharpest counterargument to frictionless shipping. Their talk, titled *The Friction Is Your Judgment*, argues for adding some friction back — not because speed is bad, but because some friction is judgment in disguise.
...
Those moments are easy to misclassify as inefficiency, especially when generation tools make everything else feel instant. Ronacher's claim is not nostalgia for slowness: the pause is where the judgment is, the point where experience gets inserted.
...
The phrase vibe coding is useful partly because it is a little embarrassing. It names a behavior people recognize on sight: low-spec, high-speed, intuition-led building steered by momentum rather than explicit structure.
...
For work that has to be trusted, stronger generation tools do not repair weak framing; they expose it faster, and at higher cost.

If a human engineer receives a vague task, progress may be slow enough that the ambiguity surfaces early. Questions emerge. A meeting happens. The task gets reshaped before much damage is done.
...
These questions used to be signs of seniority. For work that has to be trusted they are now the gate: with them unanswered you do not have a task, you have a prompt.
...
That is why review shifts from a step teams pass to where quality is decided:

Not review as bureaucratic blockage.
Review as quality discrimination.
Review as the moment the team asks whether the artifact is fit for purpose.
...
The human reviewer is therefore not an obstacle to AI productivity; the reviewer is how standards survive abundance. A rule of thumb, mine: when generation outruns review, the loop is not fast, it is unreviewed.
...
Once judgment becomes more valuable, the question is where to keep it. Taste kept only in a senior engineer's head is easy to lose in delegation; taste written into a harness, a spec, or a repo rule is harder to lose.
...
It would be too simple to say that humans now only direct while machines produce. People still produce a lot, and manual work is sometimes the fastest or safest path. But the direction is real. A larger share of human value moves toward:
...
The better the generators get, the more dangerous it becomes to confuse typing with thinking. What stays scarce is not producing tokens. It is directing production toward something coherent, durable, and worth keeping.
...
That is the recurring case this book will call the software factory. It matters because it shows where the leverage really sits. The breakthrough is not that the model suddenly became a genius, but that the team stopped treating its own tacit judgment as invisible infrastructure.
...
Once you think in harnesses, you immediately care about evaluation and runtime semantics. A harness that cannot measure quality is incomplete. A harness that cannot preserve state across longer tasks is fragile.
...
This is why the early eval set is usually seeded by hand. Samuel Colvin describes the common practice: “in general what people end up doing is they have some subset of data that’s been like human annotated.” Humans know which failures are expensive, which edge cases are recurrent, and which “good-looking” outputs are secretly wrong. Early eval discipline often begins with a human saying: this class of mistake bit us three times last month; from now on it belongs in the test set. The more a system does real work, the less synthetic evals can tell you.
...
None of this works if the system is opaque. Classic monitoring tells you whether a service is up, slow, or erroring. Agent observability has to answer a different kind of question: what did the system believe it was doing, what did it actually do, where did it drift, and what should a human now inspect?

Agent observability is not merely a nicer logging story but what makes the human control plane real. Humans cannot steer what they cannot see.
...
The field has a recurring weakness for maximalist language. Full autonomy. Fully agentic companies. End-to-end automation. One model to run the business. The mythology is understandable. Grand claims attract attention, talent, and capital. They also smuggle in bad product instincts.
...
The later chapters widened from technical to institutional systems because every serious AI question eventually turns organizational. Who maintains the harness? Who curates the eval set? Who decides what context sources count as authoritative? Who scopes permissions? Who owns review queues? Who resolves the conflicts created by private agent workflows moving faster than shared alignment? There is no lasting answer to those questions at the prompt layer.
