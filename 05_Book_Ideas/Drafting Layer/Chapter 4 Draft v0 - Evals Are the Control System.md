...
The newer observability material makes the chapter’s claim more operational. Amy Boyd and Nitya Narasimhan argue for trace-linked evaluation rather than detached scoring, because once an agent fails you need the trajectory beside the metric. Danny Gollapalli and Ben Hylak make the same point from production monitoring: the hard part is not merely scoring output, but seeing the failure family, the tool path, and the capability gap that produced it. Samuel Colvin’s production optimization material pushes further still: evals, traces, comparisons, and failure analysis increasingly belong in one loop.
...
3. preserve the relevant traces and trajectory context
4. label and structure those patterns into reusable datasets
5. compare prompts, tools, models, or workflows against them
...
This is also why Hetzel insists that “an eval platform is not just a test runner.” A runner executes checks. A real platform also stores datasets, versions scoring logic, links judgments back to traces, supports comparisons, surfaces disagreements, and creates enough trust in results that teams will actually use them to make decisions. In mature AI engineering, the platform around evaluation becomes part of the product-development process itself.
