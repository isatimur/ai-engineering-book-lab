...
- **Supporting sources:**
  ...
  - [[230-OkEGJ5G3foU-full-workshop-reinforcement-learning-kernels-reasoning-quantization-agents-daniel-han|#230 — Daniel Han]] — treats RL through its operational surface: kernels, quantization, agents.
    ...
    - **Newer edition (2026-09-14):** an off-the-cuff Q&A answer with no baseline, model, or context length stated anywhere in the window. Two months later the same speaker gives the same number for a *different and non-proprietary* mechanism — *"gradient checkpointing saves memory by dramatic amounts by like 70%. Um, 70% memory reduction with no change in accuracy."* (#1013 `uIiA6DquRiE`, 2026-07-15, 01:48:18) — with a stated cost of 10–15% slower training. In a third talk he says 80%. A vendor figure that moves 70 → 80 and changes mechanism between talks should not be read as a property of Triton kernels specifically. Low urgency: the number is decorative here, and this claim rests on the four-models-in-memory sources, not on it.
...
- **Supporting sources:**
  - [[670-zTLJNHj0DeQ-why-mlx-prince-canuma-neywa-labs|#670 — Prince Canuma, Neywa Labs]] — "I want to convince you today to offload some of that subscription completely on device and then all you need to pay is your energy bill." (working demos on consumer Macs; accessibility as the driver)
    ...
    - **Currency checked (2026-09-14):** still current, and the weakest of the flags — the "today" is rhetorical address inside the talk's agenda (*"Today's agenda, of course, we're going to start with why on device."*, 00:45), not a time-bound factual assertion. It could only go stale if on-device inference retreated. This source is from the corpus's oldest batch, so all 1,023 talks added after it were swept; nothing retreats and two newer panels extend it — #1006-adjacent "Compression at the edge" (2026-08-14) describes *"this inflection point… this resurgence of the importance of local AI"*, and a 2026-07-15 local-AI panel frames the session as why local AI is the future.
...
- **Supporting sources:**
  ...
  - [[068-6guQG_tGt0o-ai-kernel-generation-what-s-working-what-s-not-what-s-next-natalie-serrino-gimlet-labs|#068 — Natalie Serrino, Gimlet Labs]] — AI-generated kernels can meaningfully speed up custom code with no human effort.
    ...
    - **Newer edition (2026-09-14):** **the 40% is one worked example, not the benchmark result.** In the same talk the headline is *"So what we can see for the standalone agent is that we see an average speed up of about 25% or 24%."* (07:31), over 250 problems on KernelBench v0.1, on Apple M4 via Metal, and the speaker calls the results "preliminary" (07:05). The 40% is a single kernel fusion (a five-op convolution/softmax/bias/scaling/sigmoid chain); a second example in the same talk hit 80%. Quoting the example as the figure overstates the method. **Later edition:** #1071 (`pOvWgX7IJsc`, Simran Arora, Together AI, 2026-08-28) extends the benchmark to multi-GPU and both confirms and bounds it — *"uh models right now are showing really promising results on single GPU uh benchmarks. So it's a ripe time to to extend it."* (21:58) — while reporting the best frontier models solve 28 of 87 multi-GPU problems with performance plateauing near 31% even as parallel samples scale, and that successes cluster on patterns well represented on the internet. That sharpens this entry's existing caveat about how far generated kernels close the gap to expert-tuned code.
...
- **Supporting sources:**
  - [[476-gFyBdBm0AGo-system-design-for-next-gen-frontier-models-dylan-patel-semianalysis|#476 — Dylan Patel, SemiAnalysis]] — "The models that we're using today are largely the same as the models that were trained in 2022"; flagships are smaller models trained for longer (GPT-4/Turbo/4o; 3.5 Sonnet smaller than Opus but better because trained better).
    ...
    - **Currency checked (2026-09-14):** the *premise* stands. Sweeps across all 333 talks added after this source for `same architecture|still (just )?a transformer|architecture has(n't) changed|new architecture|architecturally`, `\b2022\b`, and `stagnat|plateau|hit a wall|scaling laws are` produced no source saying today's flagships are not largely 2022-era. The architecture hits fail the test on reading: DeepMind's text diffusion self-describes as "a more forward-looking research area" and "a research preview"; diffusion Gemma is cited as an inference trick. The strongest churn hit actually confirms Patel — *"every model was dense the transformer plus plus… it's an old transformer. Okay. Plus RMS lay plus some extra tricks… And then now it's like oh my it's like transformer plus+ plus"* (#1006-batch `J4_jCrTxMkk`, 35:25) — increments on the 2022 transformer, by the speaker's own framing. **Date the "today":** Patel pins it himself to roughly mid-2024 — *"3.5 CET came out recently but again that's actually smaller than Opus but it's somehow better because they trained it for longer right but we haven't seen a extremely large Model come out yet and and but we will soon"* (00:46). "We will soon" is an unfulfilled prediction two years before the reader's present.
- **Caveats / counterevidence:** Single-source; Patel's analysis, not a settled consensus. Some argue the plateau is real and architectural, not budgetary. **The corroborating source this caveat asked for was searched on 2026-09-14, and what turned up points the other way.** Patel's premise (same architecture, gains from training longer) survives, but his *resolution* — that the leap is still available and merely costs a training run nobody has finished paying for — now has two newer named sources against it. Sara Hooker (Adaption Labs, `XEd_SRVHBgU`, 2026-08-14): *"my argument is in no frontier AI lab is going to four exercise their model again for pre-training"* (18:44), *"we are probably at the ceiling of size"* (18:59), and *"again we've hit the ceiling and so it's almost like no one is going to supersize their model or if they do it's not clear it's beneficial"* (19:42). Daniel Han (`uIiA6DquRiE`, 2026-07-15): *"scaling intelligence in terms of parameters probably has plateaued most likely"* (01:41:12). Treat the budget-vs-ceiling question as contested, not settled, and do not let this claim carry public weight on its own.
- **Correction (2026-08-28):** support level lowered from `strong` during the book-2 claim-support audit. The caveat already conceded the weakness in prose; the level field did not carry it. See `research_passes/2026-08-27-book2-claim-support-audit.md`.
- **Candidate chapters:** 3
...
- **Supporting sources:**
  ...
  - [[137-mQ7_Zje7WKE-the-2025-ai-engineering-report-barr-yaron-amplify|#137 — Barr Yaron, Amplify]] — 2025 survey (n=500): evaluation named the single most painful part of AI engineering. (Supplies the practitioner-side fact — the field cannot measure well — not the steering claim itself.)
    ...
    - **Newer edition (2026-09-14):** #137 is the **2025** edition of an annual survey. The same speaker's **2026** edition is in the corpus as #906 (`RGe6EjucbzI`, added 2026-07-27). It **confirms** the finding — *"Every single year that I ask this, the answer, the number one answer is eval."* (12:09) — and adds a qualifier the 2025 quote cannot carry: *"so Eval's lead here is same as always, but by a very thin margin, like that margin is getting smaller"* (12:14). Cite the 2025 figure as a 2025 datapoint; if the prose leans on eval being the *dominant* pain rather than the *top* one, use #906. Book 1 marked the same source on 2026-09-04 for a different figure; this ledger had not.
...
- **Supporting sources:**
  - [[890-bUJgirn4_yc-when-agents-meet-physical-data-the-other-physics-of-agent-harnesses-dmitry-petrov-datachai|#890 — Dmitry Petrov, DataChain]] — "Anthropic published that accuracy for data projects on their agents is only 21% until you add specific data harnesses to them and provide context."
    ...
    - **Currency checked (2026-09-14):** no supersession and no later edition — `21 ?%` across all 1,074 talks returns exactly two hits, this one and an unrelated content-moderation rate; Krieger's Anthropic talk (2026-08-28, the likeliest home for an update) never mentions accuracy or data projects. **But the figure is unpinned in a way the entry should state.** The tape gives no publication name, no benchmark, no model, and no date beyond "this year" — and crucially no post-harness number, so the reader is told 21% is the floor but never what the harness buys: *"Anthropic published that accuracy for data projects on their agents is only 21% until you add specific data harnesses to them and provide context."* (00:43). It is also a second-hand relay of another lab's result, which the caveat already concedes.
...
- **Supporting sources:**
  - [[406-pPvoLjYj_mY-evaluating-domain-specific-llms-for-real-world-finance-waseem-alshikh-writer|#406 — Alshikh, Writer]] — a finance benchmark (FAIL) that separates answer-rate from grounding; even the top model's combined grounding-plus-answer score is about 81% (≈one in five wrong under imperfect context), used to argue general models alone are insufficient.
    ...
    - **Currency checked (2026-09-14):** still current — `81 ?%` across the corpus returns three hits, only this one on topic, and sweeps for `grounding score|hallucination rate|domain-specific models are|general models now match` across every newer talk found nothing that overtakes it. **Pin the context when citing:** the benchmark is FAIL, Writer's own, financial-services domain; the metric is robustness *and* context grounding combined, not an accuracy score; the newest models named on tape are o1 and o3 (08:06), and one name is garbled in the transcript, so the model list is incomplete. **The tape never says which model reached 81%** — "the top model" is as specific as the source gets. Alshikh time-bounds it himself, which is stronger than the ledger asserting it: *"what we start seeing believe at least today with the technology we have with the current model we have until we have something completely different"* (10:35).
